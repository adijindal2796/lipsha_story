...
def save_session():
    st.query_params.update({"s": st.session_state.session_id})
...
def init_state():
    query_session = st.query_params.get("s")
    if query_session:
    ...
    if "reading_in_progress" not in st.session_state:
        ...
        if start_new_reading:
            ...
            imgs = random.sample(
                [str(x) for x in IMAGE_DIR.iterdir() if x.name != "Aditya.png"], k=4
            )
            st.session_state.header_images = [imgs[0], imgs[1], imgs[2]]
            st.session_state.Aditya_image = str(IMAGE_DIR / "Aditya.png")
...
def main():
    ...
    _, c1, _ = st.columns(3)
    c1.image(st.session_state.Aditya_image)
    del c1
...
try:
    if "flagged_input" in st.session_state:
        ...
        st.caption("View on [Github](https://github.com/msull/Adityatarot)")
